...
content_raw = requests.get(url + '/clips;jsessionid=' + sessionid + '?filter=and((catalog.id)eq({}))&include=userFields'.format(catalog_id[4][1]))
...
#content_iv_no = [i['userFields']['U7'] for i in content_data['data']['items']]
def power_iv_gen():
	count = 0
...
#for i in range(11):
content_iv_no = [next(pow_iv) for i in range(len(content_data['data']['items']))]	
